function q=QuestRecomputeLogistic(q, plotIt)

%% Modified to accomodate logistic pmf by RT based on QuestRecompute
...
if nargin < 1
	error('Usage: q=QuestRecomputeLogistic(q [,plotIt=0])')
end
...
if length(q)>1
	for i=1:length(q(:))
		...
		q(i)=QuestRecomputeLogistic(q(i));
	end
	return
end
...
% THIS IS WHERE RT MADE MODIFICATION (Logistic)
q.p2 = 1./(1+exp(-(q.beta*q.x2)));
...
% MODIFICATION
% By definition xThreshold is always 0 (given pThreshold = 0.5)
% I am assuming symmetric experiment where pThreshold = 0.5 always
...
% MODIFICATION
% In the original code with Weibull, this compute p2 first to find
% xThreshold (because getting it in a closed form is hard, I guess). But in
% our case where cumulative gaussian is used, there is no need to do so
% because we a priori know that xThreshold = 0.
% ->> UNLESS you reintroduce gamma and delta which is allowed to be
% asymmetric (-> which is a weird situation in discrimination experiments)
...
for k=1:q.trialCount
    ...
    % Bayesian update: current estimate of pdf (q.pdf) = Prior P(T)
    % Psychometric funtion shifted according to the stimulus intensity and
    % flipped according to the response (q.s2(q.response(k)+1,ii)) = 
    % Likelihood function of T given data P(T|d)
    % Product of the two = posterior
