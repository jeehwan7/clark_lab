function q=QuestCreateLogistic(tGuess,tGuessSd,pThreshold,beta,delta,gamma,grain,range,plotIt)

%% Modified to accomodate logistic pmf by RT based on QuestCreate
...
% Beta is interpreted as k of sigmoid psychometric curve. I am too lazy to go
% around changing all the variable names across functions...
% p = 1/(1+exp(-chi)), chi = k(x-T)
q.beta=beta;

% These parameters are legacy from original function and is not going to
% be used. (I could make use of these though - but delta should be equal to gamma anyways)
q.delta=delta;
q.gamma=gamma;
% This will be ignroed too - pThreshold should be always 0.5.
q.pThreshold=pThreshold;

q.grain=grain;
q.dim=dim;
q=QuestRecomputeLogistic(q, plotIt);
