function accumulativeMeanDTheta = createAccumulativeMeanDThetabyTrial(nanThreshold, numTrials, dThetabyTrialWithoutSaccades, coherences, parities, types)
    ...
    accumulativeMeanDTheta = NaN(numTrials,range);
    ...
    for i = 1:numTrials
        ...
        if sum(isnan(dThetabyTrialWithoutSaccades(i,:)))>nanThreshold
            accumulativeMeanDTheta(i,:) = NaN(1,range);
        end
    end
    ...
    for k = 1:numTrials
        if strcmp(types(k), 'converging') && parities(k) == 1
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0.4940 0.1840 0.5560]); % purple
        end
        hold on
    end
    hold off
    ...
    for k = 1:numTrials
        if strcmp(types(k), 'converging') && parities(k) == -1
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0.4660 0.6740 0.1880]); % green
        end
        hold on
    end
    hold off
    ...
    for k = 1:numTrials
        if strcmp(types(k), 'diverging') && parities(k) == 1
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0.6350 0.0780 0.1840]); % red
        end
        hold on
    end
    hold off
    ...
    for k = 1:numTrials
        if strcmp(types(k), 'diverging') && parities(k) == 1
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0.3010 0.7450 0.9330]); % blue
        end
        hold on
    end
    hold off
    ...
    for k = 1:numTrials
        if coherences(k) == 0
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0 0 0]); % black
        elseif coherences(k) == 0.2
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0.8500 0.3250 0.0980]/2); % brown
        elseif coherences(k) == 1
            plot(1:range,accumulativeMeanDTheta(k,:),'color',[0.8500 0.3250 0.0980]); % orange
        end
        hold on
    end
    hold off
