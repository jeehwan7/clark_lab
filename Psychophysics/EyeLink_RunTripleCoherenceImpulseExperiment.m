...
% Save Results File
if ~isfolder('./triplecoherenceimpulseresults'); mkdir('./triplecoherenceimpulseresults'); end
startTime = datestr(now,'yyyy.mm.dd_HHMM');
mkdir(['./triplecoherenceimpulseresults/','Subject',num2str(subjectID),'_',startTime]);
save(['./triplecoherenceimpulseresults/','Subject',num2str(subjectID),'_',startTime,'/','Subject',num2str(subjectID),'_',startTime,'.mat'],'subjectID','startTime','param','pxPermm','screenWidthpx');

% Create EyeLink Data Folder (edf files)
if ~isfolder(['./triplecoherenceimpulseresults/','Subject',num2str(subjectID),'_',startTime,'/EdfFiles'])
    mkdir(['./triplecoherenceimpulseresults/','Subject',num2str(subjectID),'_',startTime,'/EdfFiles']);
end
...
for ii = 1:param.numBlocks
    ...
    for jj = 1:param.numTrialsPerBlock
        [tripleSquares, dir] = triple(numSquaresX, numSquaresY, numFrames, param.parity, param.diverging, param.fracCoherence);
...
save(['./triplecoherenceimpulseresults/','Subject',num2str(subjectID),'_',startTime,'/','Subject',num2str(subjectID),'_',startTime,'.mat'],'stimuli','directions','results','-append');
...
% 3D Matrix for Triple Pattern with Randomly Switching Directions
function [mt, dir] = triple(x, y, z, par, div, fracCoherence)

    dir = 2*randi([0 1],[1 z-1])-1;

    % if diverging, flip the direction matrix (without the initial NaN) first
    if div == 0
        temp_dir = dir;
    elseif div == 1
        temp_dir = flip(dir, 2);
    ...
    temp_dir = [NaN, temp_dir];

    % first frame
    mt(:,:,1) = (zeros(y,x)-1).^(randi([0 1],[y,x]));
    
    for t = 2:z
        if temp_dir(1,t) == 1 % right
            mt(:,1,t) = (zeros(y,1)-1).^(randi([0 1],[y,1])); % new leftmost column
            mt(:,2:x,t) = par*mt(:,1:x-1,t-1).*mt(:,2:x,t-1); % apply correlation
        elseif temp_dir(1,t) == -1 % left
            mt(:,end,t) = (zeros(y,1)-1).^(randi([0 1],[y 1])); % new rightmost column
            mt(:,1:x-1,t) = par*mt(:,1:x-1,t-1).*mt(:,2:x,t-1); % apply correlation
        end
    end

    if div == 1 % have to flip twice for diverging
        mt = flip(flip(mt,2),3);
    end
    ...
    for t = 2:z
        ...
        mt(x*y*(t-1)+indexRandom) = 2*(rand(1,size(indexRandom,2))>0.5)-1;
    end
