function Q = removeSaccades(Q, threshold, cutTime)
    ...
    for ii = 1:Q.numTrials
        ...
        if abs(temp(ii,duration-1))>threshold
            temp(ii,duration-cutTime:duration-1) = NaN(1,cutTime);
        end
    end

    Q.eyeVelocityWithoutSaccades = temp(:,1:duration-1); % cut off at stimulus duration
