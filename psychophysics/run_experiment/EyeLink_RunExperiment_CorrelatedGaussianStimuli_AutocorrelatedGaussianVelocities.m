...
% Save Results File
if ~isfolder('./gaussianAutocorrelatedGaussianresults'); mkdir('./gaussianAutocorrelatedGaussianresults'); end
startTime = datestr(now,'yyyy.mm.dd_HHMM');
mkdir(['./gaussianAutocorrelatedGaussianresults/','Subject',num2str(subjectID),'_',startTime]);
save(['./gaussianAutocorrelatedGaussianresults/','Subject',num2str(subjectID),'_',startTime,'/','Subject',num2str(subjectID),'_',startTime,'.mat'],'subjectID','startTime','param','pxPermm','screenWidthpx');

% Create EyeLink Data Folder (edf files)
if ~isfolder(['./gaussianAutocorrelatedGaussianresults/','Subject',num2str(subjectID),'_',startTime,'/EdfFiles'])
    mkdir(['./gaussianAutocorrelatedGaussianresults/','Subject',num2str(subjectID),'_',startTime,'/EdfFiles']);
end
...
for ii = 1:param.numBlocks
    ...
    for jj = 1:param.numTrialsPerBlock
        ...
        for kk = 2:numFrames
            prev = init(:,:,kk-1);
            stim(:,:,kk) = alpha*init(:,:,kk) + beta*circshift(prev,g(kk),2);
        end
...
save(['./gaussianAutocorrelatedGaussianresults/','Subject',num2str(subjectID),'_',startTime,'/','Subject',num2str(subjectID),'_',startTime,'.mat'],'stimVelocity','results','-append');
