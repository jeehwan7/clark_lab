...
% Save Results File
if ~isfolder('./binaryCorrelatedresults'); mkdir('./binaryCorrelatedresults'); end
startTime = datestr(now,'yyyy.mm.dd_HHMM');
mkdir(['./binaryCorrelatedresults/','Subject',num2str(subjectID),'_',startTime]);
save(['./binaryCorrelatedresults/','Subject',num2str(subjectID),'_',startTime,'/','Subject',num2str(subjectID),'_',startTime,'.mat'],'subjectID','startTime','param','pxPermm','screenWidthpx');

% Create EyeLink Data Folder (edf files)
if ~isfolder(['./binaryCorrelatedresults/','Subject',num2str(subjectID),'_',startTime,'/EdfFiles'])
    mkdir(['./binaryCorrelatedresults/','Subject',num2str(subjectID),'_',startTime,'/EdfFiles']);
end
...
for ii = 1:param.numBlocks
    ...
    for jj = 1:param.numTrialsPerBlock
        ...
        % correlated gaussian stimulus velocities
        ...
        init = rand(numSquaresY,numSquaresX);
        init = (init>0.5)*2-1;
...
save(['./binaryCorrelatedresults/','Subject',num2str(subjectID),'_',startTime,'/','Subject',num2str(subjectID),'_',startTime,'.mat'],'stimVelocity','results','-append');
...
function g = correlatedGaussian(numFrames,corrFrames) % actually numFrames-1
    ...
    g = alpha*g + beta*circshift(g,corrFrames,1);
